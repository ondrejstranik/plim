...
fMName = 'Exp1'
norFolder = r'F:\ondra\LPI\plim\DATA\ifcBased\26-03-16 highMag_BSA\rawNorm'
savFolder =  r'F:\ondra\LPI\plim\DATA\ifcBased\26-03-16 highMag_BSA\rawShift'
...
for ii, (_fn, _ft) in enumerate(zip(fileName,fileTime)):
    ...
    if ii==0:
        imCube = np.zeros((len(fileName),image.shape[1],image.shape[2]))
...
aligned_stack, shifts = align_stack_unmasked(imEdge, upsample_factor=100)
imCubeNN = imCube*1
...
for ii, (_fn, _ft) in enumerate(zip(fileName,fileTime)):
    try:
        ...
        print(f'image {ii} out of {nFile}')
        sImage = ndi_shift(image, shift=(0,*shifts[ii]), mode='constant', cval=0)
        sVideo.saveImage(sImage,_ft)
    except:
        print('could not load the file')
